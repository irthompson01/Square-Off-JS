...
function newConnection(socket) {
  ...
  socket.on("disconnect", function () {
    ...
    if (clients[socket.id] != null) {
      ...
      console.log(
        "Client removed.\tNumber of clients: " + Object.keys(clients).length,
      );
  ...
  function onClientConnect(data) {
    if (rooms[data.roomId] != null) {
      console.log(
        "clientConnect message received from " +
          socket.id +
          " for room " +
          data.roomId +
          ".",
...
const roomNames = JSON.parse(fs.readFileSync("./roomIds.json", "utf8"));
