...
export function resetScoreDisplay(board) {
  board.players.forEach((player) => {
    ...
    document.getElementById(scoreDisplayId).innerText =
      player.getScoreDisplay();
    document.getElementById(multDisplayId).innerText =
      player.getMultiplierDisplay();

    // Highlight the current player
    if (player === board.current_player) {
      document.getElementById(playerDivId).style.backgroundColor =
        player.fillStyle;
...
export function updateScoreDisplay(board) {
  board.players.forEach((player) => {
    ...
    if (
      board.players.indexOf(player) ==
      (board.players.indexOf(board.current_player) + 1) % board.players.length
    ) {
      document.getElementById(playerDivId).style.backgroundColor =
        player.fillStyle;
      document.getElementById(playerNameId).style.color = "#ffffff";
