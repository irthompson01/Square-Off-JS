...
export class Board {
  ...
  findNewBoxes() {
    ...
    this.squares.forEach((square) => {
      if (square.checkOwnership() == true) {
        ...
        square.bottomRight.sprite.rotate(
          this.rotationAngle,
          this.rotationSpeed,
        );

        let s = new this.squareSprites.Sprite(
          square.topLeft.origin_x + (this.tile_length_px * square.size) / 2,
    ...
    this.diamonds.forEach((diamond) => {
      if (diamond.checkOwnership() == true) {
        ...
        let d = new this.squareSprites.Sprite(
          diamond.top.origin_x + this.tile_length_px / 2,
          diamond.top.origin_y +
            this.tile_length_px * (diamond.size - 1) +
            this.tile_length_px / 2,
  ...
  updateScore() {
    ...
    for (var i = 0; i < mult; i++) {
      ...
      this.current_player.scoreIncrease += Math.round(
        this.current_player.getMultiplier() * points,
      );
  ...
  createGrid(size) {
    ...
    for (var i = 0; i < this.width; i++) {
      for (var j = 0; j < this.height; j++) {
        ...
        grid[i][j] = new Tile(
          this.origin_x + this.tile_length_px * j,
